...
export function GameContent(props:PropsType) {
  ...
  const joinGame = async (roomJson:roomType) => {
    ...
    if(isSignedIn && roomJson){
      console.log('FF')
      if(roomJson.status=='waiting_for_player' && user.id !== roomJson.players.player1.id){
        //Rejoin la partie en temp que player2

        console.log('tentative')

        let couleur:PieceColor = roomJson.players.player1.color=='white'?'black':'white'

        let newPlayers = roomJson.players
        newPlayers.player2 = {
          id:user.id,
          color:couleur,
        ...
        if(user.id==roomJson.players.player1.id){
          ...
          console.log('df')
          setGameInfos(roomJson.players.player1.color,parseInt(roomJson.cadence.split('|')[0]))
        }else if(user.id==roomJson.players.player2.id){
          console.log(roomJson.players.player2)
          console.log('z')
          setGameInfos(roomJson.players.player2.color,parseInt(roomJson.cadence.split('|')[0]))
        }
